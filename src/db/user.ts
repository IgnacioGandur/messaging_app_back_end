...
class User {
    ...
    async getUserByUsername(username: string, omitPassword: boolean = false) {
    ...
    ): Promise<UserType | UserWithoutPassword | null> {
        const user = await this.prisma.user.findUnique({
            where: {
                id: Number(id),
            },
    ...
    async getFullUserProfile(id: number | string) {
        const [user, ownedGroups, friendships, joinedGroups] =
    ...
    async getAllUsers(pageSize: number, skip: number, search: string) {
        try {
        ...
        const user = await this.prisma.$transaction(async (tx) => {
            ...
            for (const conv of ownedConversations) {
                if (conv.isGroup) {
                    ...
                    const newOwner = conv.participants.find(
    ...
    async getUserWithStats(userId: number | string) {
