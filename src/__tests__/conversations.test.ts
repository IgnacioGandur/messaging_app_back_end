...
describe("Conversations Route.", () => {
    ...
    it("POST | Should create a private conversation between two users.", async () => {
        ...
        expect(response.body.conversation.participants.some((p: { userId: string | number }) => p.userId === john_doe!.id)).toBe(true);
        expect(response.body.conversation.participants.some((p: { userId: string | number }) => p.userId === jane_doe!.id)).toBe(true);

        // The sended message should exist.
        expect(response.body.conversation.messages.some((m: { content: string }) => m.content === "Sample message!")).toBe(true);
    });
    ...
    it("POST | Should fail creating a private conversation due to recipient not existing.", async () => {
        ...
        expect(response.body.errors.some((e: { msg: string }) => e.msg === "The recipient id field should be an integer.")).toBe(true);
        expect(response.body.errors.some((e: { msg: string }) => e.msg === "The message field can't be empty.")).toBe(true);
    });
});
